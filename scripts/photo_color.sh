...
cat > scripts/photo_color.sh <<'EOF'
#!/bin/bash
...
RESPONSE=$(curl -s -X POST \
    "http://127.0.0.1:8000/api/photo/color-correct?brightness=$BRIGHTNESS&contrast=$CONTRAST&saturation=$SATURATION&auto_white_balance=$AUTO_WB" \
    -F "file=@$INPUT")
